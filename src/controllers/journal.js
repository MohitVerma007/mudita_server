...
exports.postJournalEntry = async (req, res) => {
  try {
    ...
    UPDATE mentees SET current_score = $1
    WHERE user_id = $2
    RETURNING *;
  `;
    ...
    if (existingRows.length === 0) {
      ...
      const { nrow } = await db.query(updatementeeQuery, [
        user_id,
        req.body.current_score,
      ]);
      console.log(nrow, "success first");
      ...
      `;
        ...
        const { nrow } = await db.query(updatementeeQuery, [user_id, avgscore]);
        console.log(nrow, "success avg");
      ...
      `;
        ...
        console.log(nrow, "success update pv");
  ...
  try {
    ...
    if (existingRows.length === 0) {
      ...
      if (existingEntry.month === req.body.month) {
        ...
        const { nrow } = await db.query(updatementeeQuery, [avgscore, user_id]);
        console.log(nrow, "success avg");
